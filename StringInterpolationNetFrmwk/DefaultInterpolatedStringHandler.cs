...
#if !CSHARP10FORLEGACY
#define CSHARP10FORLEGACY
...
namespace System.Runtime.CompilerServices
{
    ...
    public ref struct DefaultInterpolatedStringHandler
    {
        ...
        private const int MinimumArrayPoolLength = 255;

        // Limited by the CLR
        private const int MaxStringLength = 1073741791;
        ...
        {
            _provider = null;
            _chars = _arrayToReturnToPool = CharArrayPool.Rent(GetDefaultLength(literalLength, formattedCount));
            _pos = 0;
        ...
        public override string ToString() => new(_chars, 0, _pos);
        ...
        {
            string result = new(_chars, 0, _pos);
            Clear();
            return result;
        }
        ...
        private void AppendStringDirect(string value)
        {
            ...
            if ((uint)pos < chars.Length - length + 1)
            {
...
#region AppendFormatted string
        ...
        public void AppendFormatted(string value)
        {
            ...
            if (!_hasCustomFormatter &&
                value != null &&
                (uint)pos < chars.Length - length + 1)
            {
                for (int i = 0; i < length; i++)
                    chars[pos + i] = value[i];
                _pos += length;
        ...
        private void AppendFormattedSlow(string value)
        {
            ...
            else if (value != null)
            {
                ...
                EnsureCapacityForAdditionalChars(length);
...
#region AppendFormatted object
        ...
        private void AppendOrInsertAlignmentIfNeeded(int startingPos, int alignment)
        {
            ...
            if (paddingNeeded > 0)
            {
                ...
                if (leftAlign)
                {
                    for (int i = pos; i < pos + paddingNeeded; i++)
                        chars[i] = ' ';
                }
                else
                {
                    // Move already copied characters over to make room for padding
                    // then overwrite with padding
                    for (int i = startingPos; i < startingPos + paddingNeeded; i++)
                    {
                        chars[i + paddingNeeded] = chars[i];
                        chars[i] = ' ';
                    }
                }
        ...
        private void GrowCore(uint requiredMinCapacity)
        {
            ...
            int arraySize = MinimumArrayPoolLength < newCapacity 
                ? (newCapacity < int.MaxValue ? (int)newCapacity : int.MaxValue)
                : MinimumArrayPoolLength;

            char[] newArray = CharArrayPool.Rent(arraySize);
            Array.Copy(_chars, newArray, _pos);
    ...
    {
        // Interpolator wont ask for an array smaller than 255 (Log2 of 255 is 7), so 
        // that's 8 array buckets that dont need to be allocated.
        // Only pooling arrays up to 1MB, which is 2^20, so Log2 is 20.  Subtract 7
        // buckets for the smaller than 255 arrays that wont be created, and NumberOfBlocks is 13.
        private const int MaxLogValueStored = 20;
        private const int NumberOfBlocks = MaxLogValueStored - 7;
        private static readonly LockedStack[] arrays = new LockedStack[NumberOfBlocks];

        static CharArrayPool()
        {
            arrays[0] = new LockedStack(20);
            arrays[1] = new LockedStack(10);
            arrays[2] = new LockedStack(5);
            arrays[3] = new LockedStack(5);
            arrays[4] = new LockedStack(5);
            arrays[5] = new LockedStack(5);

            for (int i = 6; i < NumberOfBlocks; i++)
            {
                arrays[i] = new LockedStack(2);
            }
        }

        internal static char[] Rent(int minimumLength)
        {
            // Dont need to check for 0 length - Interpolator wont ask for less than a 256 length array.

            int index = Log2((uint)minimumLength);
            if ((uint)index < MaxLogValueStored)
            {
                LockedStack bucket = arrays[index - 7];
                char[] charArray = bucket.TryPop();
                // Subtract 1 so that the allocated value is 1 lower than the next tier Log2
                // value, so that it gets returned to the correct bucket.
                return charArray ?? (new char[(1 << (index + 1)) - 1]);
            }
            else // index >= NumberOfArrays
            {
        ...
        {
            // Dont need to check for null as Interpolator is doing it

            int index = Log2((uint)array.Length);
            if ((uint)index < MaxLogValueStored)
            {
                LockedStack bucket = arrays[index - 7];
                bucket.TryPush(array);
            }
        }

        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        internal static int Log2(uint operand)
        {
            // Since the Interpolator will never ask for less than 255, no need to evaluate
            // the first 7 bits.  Skip them.
            operand >>= 8;
        ...
        /// <summary>Provides a simple, bounded stack of char arrays, protected by a lock.</summary>
        [DebuggerDisplay("{count} arrays available{SizeDisplay, nq}")]
        private sealed class LockedStack
        {
            /// <summary>The arrays in the stack.</summary>
            private readonly char[][] arrays;
            /// <summary>Number of arrays stored in <see cref="arrays"/>.</summary>
            private int count;

            internal LockedStack(int capacity)
            {
                arrays = arrays = new char[capacity][];
            }

            [MethodImpl(MethodImplOptions.AggressiveInlining)]
            public void TryPush(char[] array)
            {
                Monitor.Enter(this);
                char[][] arraysLocal = arrays;
                int countOfArrays = count;
                if ((uint)countOfArrays < (uint)arraysLocal.Length)
                {
                    arraysLocal[countOfArrays] = array;
                    count = countOfArrays + 1;
                }
                Monitor.Exit(this);
            }
            ...
            public char[] TryPop()
            {
                ...
                Monitor.Enter(this);
                char[][] arraysLocal = arrays;
                int countOfArrays = count - 1;
                if ((uint)countOfArrays < (uint)arraysLocal.Length)
                {
                    arr = arraysLocal[countOfArrays];
                    arraysLocal[countOfArrays] = null;
                    count = countOfArrays;
                }
                Monitor.Exit(this);
                return arr;
            ...
            private string SizeDisplay
                => count == 0 ? string.Empty : " of size " + arrays[0].Length;
        }
    }
}
